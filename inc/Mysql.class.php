...
class Mysql
{
    ...
    {
        if (defined('QK_CHARSET'))
        {
            $charset = strtolower(str_replace('-', '', QK_CHARSET));
        }
        ...
        else
        {
			...
			{
				$arr = each($this->settings);
				echo "\$this->settings[\"$arr[key]\"] = $arr[value]<br>";
			}
			exit;
			*/
		}
    ...
    function connect($dbhost, $dbuser, $dbpw, $dbname = '', $charset = 'utf8', $pconnect = 0, $quiet = 0)
    {
        ...
        else
        {
            if (PHP_VERSION >= '4.2')
            {
				...
				//echo $this->link_id;	//Resource id #18
				//exit;
            }
            else
            {
		...
		//echo $this->version;	//5.0.22-community-nt
        ...
        if ($this->max_cache_time && $this->starttime > $this->mysql_config_cache_file_time + $this->max_cache_time)
        {
            if ($dbhost != '.')
            {
				...
				//echo $row['Value'];	//D:\Program Files\MySQL\MySQL Server 5.0\
				//echo $row['Value']{3};	//P
				//exit;
            }
            else
    ...
    function query($sql, $type = '')
    {
        ...
        if (!($query = mysql_query($sql, $this->link_id)) && $type != 'SILENT')
        {
            ...
            $this->ErrorMsg('{"success":false,"message":"DataBase Query Error","type":2,"info":"'.$sql."->".mysql_error($this->link_id).'"}');
